...
def train():
    ...
    # Optionally load from a checkpoint - find the latest one automatically
    def find_latest_checkpoint(checkpoints_dir):
        ...
        for file in checkpoint_files:
            try:
                epoch_num = int(os.path.basename(file).split('_')[2].split('.')[0])
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        with torch.no_grad():
            ...
            # Take first 4 samples in batch
            num_samples = min(4, sar.size(0))
            all_images = []
            
            for i in range(num_samples):
                sar_img = (sar[i] + 1) / 2
                color_img = (color[i] + 1) / 2
                fake_img = (fake[i] + 1) / 2
                # Stack horizontally for each sample: SAR, GT, Generated
                sample_grid = torch.stack([sar_img, color_img, fake_img], dim=0)
                all_images.append(sample_grid)
            
            # Stack all samples vertically, then create grid
            final_grid = torch.cat(all_images, dim=0)
            grid = make_grid(final_grid, nrow=3)
